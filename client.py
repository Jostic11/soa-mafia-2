...
class UnaryClient:
    def __init__(self, host, port, players):
        ...
        self.parameters = pika.ConnectionParameters(host='localhost', port=5672, credentials=self.credentials, heartbeat=600,
    ...
    def subscribe_to_notifications(self):
        ...
        try:
            for notification in self.stub.SubscribeToNotifications(
                ...
                elif notification.msg.split()[0] == "__MSG__":
                    st = 1
                    if notification.msg.split()[1] == "__MAFIA__":
                        if self.role != "mafia":
                            continue
                        st = 2
                    if self.in_chat:
                        print(" ".join(notification.msg.split()[st:]))
                    continue
    ...
    def run_night(self, city):
        ...
        if self.role is not None:
            if self.role == "mafia":
                print("Now you are in a chat with mafia, after you chat, send an empty message and you will continue to play")
                self.in_chat = True
                while True:
                    tmp = str(input())
                    if tmp == "":
                        break
                    tmp = f"__MAFIA__ User {self.name} send: " + tmp
                    self.channel.basic_publish(exchange='', routing_key=self.game_id, body=tmp)
                self.in_chat = False
    ...
    def run_day(self, city):
        ...
        response = None
        if self.role is not None:
            print("Now you are in a chat, after you chat, send an empty message and you will continue to play")
